...
<div
    class="bg-white border border-gray-200 rounded-2xl shadow-md mb-6 hover:shadow-lg transition-all duration-300 overflow-hidden group">
        ...
        <div class="flex-1 p-6 flex flex-col justify-between">
            ...
            <div>
                <a href="{{ route('posts.show', ['username' => $post->user->username, 'post' => $post->slug]) }}">
                    <h2
                        class="text-2xl font-semibold text-gray-900 group-hover:text-indigo-600 transition-colors duration-200">
                        {{ $post->title }}
                    </h2>
                </a>
            ...
            <div class="mt-5 flex flex-col sm:flex-row sm:items-center sm:justify-between text-sm text-gray-500 gap-3">
                ...
                <div class="flex items-center">
                    <img src="{{ $post->user->imageUrl() ?? asset('images/default-avatar.png') }}"
                        alt="{{ $post->user->name }}"
                ...
                <div class="flex items-center space-x-2">
                    ...
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M6.633 10.25c.806 0 1.533-.446 2.031-1.08a9.041 9.041
                    ...
                    <!-- Count -->
                    <span class="text-gray-700 font-medium">{{ $post->claps()->count() }}</span>
                    <span class="text-gray-400 text-sm">Claps</span>
                </div>
            </div>
        </div>
